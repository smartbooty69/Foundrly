...
const ChatView: React.FC<ChatViewProps> = ({ chatId, onGoBack, currentUserId }) => {
  ...
  const [showReactions, setShowReactions] = useState<{ [msgId: string]: boolean }>({});
                  ...
                  <div className="group flex flex-col items-start relative">
                    <div className="flex items-center relative">
                      <div 
                        ...
                        onClick={() => setShowReactions(r => ({ ...r, [msg.id]: false }))}
                      >
                        {msg.text}
                        {/* Reactions on bubble */}
                        {(() => {
                            ...
                            return (
                              <div className="absolute -bottom-2 -right-1">
                                <span className="text-xs select-none px-1 py-0.5 bg-white rounded-full flex items-center shadow-sm border">
                                  {mostCommonEmoji} <span className="text-xs ml-0.5 font-medium">{count}</span>
                                </span>
                              </div>
                            );
                          }
                      ...
                      <div className="flex flex-row items-center justify-center ml-2 gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button className="p-1" onClick={() => setShowReactions(r => ({ ...r, [msg.id]: !r[msg.id] }))}><AddReactionIcon /></button>
                      </div>
                    </div>
                    {/* Emoji picker centered in chat */}
                    {showReactions[msg.id] && (
                      <>
                        <div className="fixed inset-0 bg-black bg-opacity-20 z-40" onClick={() => setShowReactions(r => ({ ...r, [msg.id]: false }))} />
                        <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2 bg-white rounded-2xl shadow-lg px-3 py-2 z-50 items-center" onClick={(e) => e.stopPropagation()}>
                          {EMOJI_REACTIONS.map((emoji) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setShowReactions(r => ({ ...r, [msg.id]: false }));
                              }}
                            >
                              {emoji}
                            </button>
